...
export const FAQSection = () => {
  ...
  return (
      <SectionLayout innerLayerClassName="px-[10vw] md:px-[5vw]">
        <div className="w-full h-full flex flex-col gap-20 md:gap-10">
          <h3 className="text-2xl md:text-4xl font-extrabold text-left dark:text-white">
            FAQs
          </h3>
...
const FAQAccordion = ({ question, answer }) => {
  ...
  return (
      <div className="flex flex-col rounded-xl px-5 py-4 md:px-10 md:py-8 shadow-xl w-full gap-4 md:gap-6 dark:bg-neutral-400">
        <div className="flex justify-between items-center gap-4 md:gap-6">
          <p className="text-sm md:text-lg font-bold truncate text-left">
            {question}
          </p>
          <button
              className="rounded-full shadow-md transition border border-gray-300 p-1"
              onClick={() => {
                setIsOpen(!isOpen);
              }}
          >
        ...
        >
          <p className="text-sm md:text-base text-left mt-2 md:mt-4">{answer}</p>
        </div>
      </div>
  );
};